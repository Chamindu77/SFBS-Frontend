...
//         try {
//             // Send PUT request to reject the request
...
//                 // Optionally refresh the list of requests here by refetching the data
...
const TableComponent = () => {
    ...
    useEffect(() => {
        const fetchSessionRequests = async () => {
            try {
                const response = await axios.get('https://sfbs-backend.vercel.app/api/v1/session/coach/requests', {
                    headers: {
                        'x-auth-token': localStorage.getItem('token'),
                    },
                });
    ...
    const fetchAvailableCourts = async (request) => {
        ...
        try {
            ...
            const response = await axios.post('https://sfbs-backend.vercel.app/api/v1/facility-booking/available-facilities', {
                sportName: request.sportName,
                date: formatDate(request.requestedTimeSlots[0].date),
                timeSlot: formattedTimeSlot,
            }, {
    ...
    const handleReject = async (request) => {
        try {
            const response = await axios.put(
                `https://sfbs-backend.vercel.app/api/v1/session/respond/${request._id}`,
                {
                    status: 'Rejected',
                },
                {
